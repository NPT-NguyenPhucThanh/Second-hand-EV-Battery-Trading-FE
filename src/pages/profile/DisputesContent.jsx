...
export default function DisputesContent() {
  ...
  const fetchEligibleOrdersAndProducts = async () => {
    ...
    try {
      ...
      const productIds = [...new Set(
        eligible
          .flatMap((order) => order.details?.map(d => d.products?.productid) || [])
          .filter(Boolean)
      )];
  ...
  const productInMap = selectedProduct?.productid ? productsMap[selectedProduct.productid] : null;
    ...
    <div className="relative min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {loading ? (
          ...
          <div className={`relative overflow-hidden rounded-2xl p-12 text-center ${isDark ? "bg-gradient-to-br from-gray-800/80 to-gray-900/80" : "bg-gradient-to-br from-white/80 to-gray-50/80"} backdrop-blur-xl border ${isDark ? "border-gray-700/50" : "border-gray-200/50"}`}>
            ...
            {disputes.map((dispute, index) => {
              ...
              return (
                <div
                  ...
                  className={`relative overflow-hidden rounded-2xl border ${isDark ? "bg-gray-800/90 border-gray-700" : "bg-white border-gray-200"} shadow-xl hover:shadow-2xl transition-all duration-500 animate-fade-in`}
                      ...
                      <div className="flex-1 space-y-6">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                          <div>
                            ...
                            {product.model && (
                              <p className="text-2xl font-bold text-blue-600 mt-1">{product.model}</p>
                            )}
                        ...
                        {product.specs && (
                          <div className={`inline-flex items-center gap-3 px-5 py-3 rounded-full ${isDark ? "bg-gray-900/70" : "bg-gray-100"} border ${isDark ? "border-gray-700" : "border-gray-300"}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-300 dark:border-gray-700">
                          <div className="flex items-center gap-3">
                            <Calendar className="w-5 h-5 text-gray-500" />
                            <div>
                              <p className="text-sm text-gray-500">Ngày tạo khiếu nại</p>
                          ...
                          <div className={`p-6 rounded-xl border-l-8 border-green-500 ${isDark ? "bg-green-900/30" : "bg-green-50"} shadow-lg`}>
                            <div className="flex gap-4">
                              ...
                              <div>
                                ...
                                <p className={`mt-3 text-base leading-relaxed ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                                  {dispute.resolution}
                                </p>
                              </div>
                            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl p-8 ${isDark ? "bg-gradient-to-br from-gray-800 to-gray-900" : "bg-gradient-to-br from-white to-gray-50"} border ${isDark ? "border-gray-700" : "border-gray-200"} shadow-2xl`}>
            <button onClick={() => setShowModal(false)} className={`absolute top-4 right-4 p-2 rounded-lg ${isDark ? "hover:bg-gray-700" : "hover:bg-gray-100"} transition-colors`}>
              <X className="w-6 h-6" />
            </button>

            <h2 className={`text-3xl font-bold mb-8 ${isDark ? "text-white" : "text-gray-900"}`}>Tạo Khiếu Nại Mới</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <select
                  ...
                  required
                  className={`w-full px-5 py-4 rounded-xl text-lg font-medium border ${isDark ? "bg-gray-900/50 border-gray-700 text-white" : "bg-white border-gray-300 text-gray-900"} focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all`}
                >
                  ...
                  {eligibleOrders.map((order) => {
                    ...
                    const price = (order.totalfinal || order.totalamount || 0).toLocaleString("vi-VN");
                    return (
              ...
              {selectedOrder && selectedProduct && (
                <div className={`p-6 rounded-xl border-2 ${isDark ? "bg-gray-900/70 border-gray-600" : "bg-gray-50 border-gray-300"}`}>
                  <div className="flex gap-6">
                    <div className="flex-shrink-0">
                      {selectedProduct.imgs?.[0]?.url ? (
                        <img src={selectedProduct.imgs[0].url} alt={selectedProduct.productname} className="w-40 h-40 object-cover rounded-xl shadow-xl" />
                      ) : (
                    ...
                    <div className="flex-1">
                      ...
                      {selectedProduct.model && <p className="text-xl text-blue-600 font-bold mt-1">{selectedProduct.model}</p>}
                      {selectedProduct.brandcars?.licensePlate && <p className="text-lg text-gray-500 mt-2">Biển số: {selectedProduct.brandcars.licensePlate}</p>}
                      {selectedProduct.specs && <p className="text-sm text-gray-600 dark:text-gray-400 mt-3 flex items-center gap-2"><Zap className="w-5 h-5" />{selectedProduct.specs}</p>}
                      <p className="text-3xl font-bold text-emerald-600 mt-6">
                        {(selectedOrder.totalfinal || selectedOrder.totalamount || 0).toLocaleString("vi-VN")}₫
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                <select
                  ...
                  required
                  className={`w-full px-5 py-4 rounded-xl text-lg border ${isDark ? "bg-gray-900/50 border-gray-700 text-white" : "bg-white border-gray-300 text-gray-900"} focus:outline-none focus:ring-2 focus:ring-orange-500`}
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full px-5 py-4 rounded-xl resize-none border ${isDark ? "bg-gray-900/50 border-gray-700 text-white placeholder-gray-500" : "bg-white border-gray-300 text-gray-900 placeholder-gray-400"} focus:outline-none focus:ring-2 focus:ring-orange-500`}
                />
              </div>

              <div className={`flex items-start gap-4 p-5 rounded-xl ${isDark ? "bg-yellow-500/20 border-yellow-500/30" : "bg-yellow-50 border-yellow-200"} border`}>
                <AlertTriangle className={`w-6 h-6 mt-0.5 flex-shrink-0 ${isDark ? "text-yellow-400" : "text-yellow-600"}`} />
                <p className={`text-sm ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                  Khi tạo khiếu nại, tiền sẽ được giữ trong ví trung gian (escrow) cho đến khi được giải quyết.
                </p>
              </div>

              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setShowModal(false)} disabled={submitting}
                  className={`flex-1 px-8 py-4 rounded-xl text-lg font-bold ${isDark ? "bg-gray-700 hover:bg-gray-600 text-white" : "bg-gray-200 hover:bg-gray-300 text-gray-900"} disabled:opacity-50 transition-all`}>
                  Hủy
                </button>
