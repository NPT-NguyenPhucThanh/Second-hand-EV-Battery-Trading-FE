...
import { Package, Warehouse, Eye, Loader2, Clock } from 'lucide-react';
...
export default function WarehousePending() {
  ...
  return (
    <div className={`min-h-screen p-6 ${isDark ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="mb-8">
        <AuroraText className="text-4xl font-bold mb-2">Kho Chờ Nhập</AuroraText>
        <p className={isDark ? "text-gray-400" : "text-gray-600"}>Sản phẩm đang chờ nhập kho</p>
      </div>
          ...
          <div key={product.productid} className={`rounded-2xl p-6 transform transition-all duration-300 hover:scale-105 ${isDark ? "bg-gray-800/50 backdrop-blur-sm" : "bg-white shadow-lg"}`}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Package className={`w-5 h-5 ${isDark ? "text-orange-400" : "text-orange-500"}`} />
                <span className={`font-bold ${isDark ? "text-white" : "text-gray-900"}`}>#{product.productid}</span>
              </div>
              <div className={`px-3 py-1 rounded-lg bg-orange-500/20 text-orange-400`}>
            ...
            <div className="space-y-2 mb-4">
              <div className="flex items-center gap-2">
                <Warehouse className={`w-4 h-4 ${isDark ? "text-gray-400" : "text-gray-500"}`} />
                <span className={`text-sm ${isDark ? "text-gray-300" : "text-gray-700"}`}>Chờ nhập kho</span>
              </div>
              {product.producttype && (
      ...
      {products.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className={`text-lg ${isDark ? "text-gray-400" : "text-gray-600"}`}>Không có sản phẩm chờ nhập kho</p>
        </div>
      )}
